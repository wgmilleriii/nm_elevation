...
class RidgeDetector:
    ...
    def generate_visualizations(self, output_dir):
        ...
        for i, method in enumerate(self.methods):
            ...
            # Save visualization
            output_path = os.path.join(output_dir, f'control_ridge_method_{i+1}.jpg')
            ...
            for x in range(self.width):
                mask[ridge[x]:, x] = 255
            cv2.imwrite(os.path.join(output_dir, f'control_ridge_mask_{i+1}.jpg'), mask)

class RidgeSelector:
    def __init__(self, image_path, ridge_points, image_name):
        self.root = tk.Tk()
        self.root.title(f"Ridge Point Selector - {image_name}")
    ...
    def save_points(self):
        """Save selected points to file with image-specific name."""
        if self.selected_points:
            filename = f'selected_ridge_points_{self.image_name}.txt'
            with open(filename, 'w') as f:
                for x, y in self.selected_points:
                    f.write(f"{x},{y}\n")
            print(f"Saved {len(self.selected_points)} points to {filename}")
            self.root.quit()
...
def main():
    ...
    image_path = os.path.join(workspace_root, "data/images/control1.png")
    output_dir = os.path.join(workspace_root, "data/images/control_ridge_detection")
    
    # Create ridge detector
    detector = RidgeDetector(image_path)
    
    # Generate visualizations
    print("Generating control image ridge detection visualizations...")
    ...
    print("\nOpening control image ridge point selector...")
    print("Direction: EAST (90°)")
    selector = RidgeSelector(image_path, detector.ridge_points, "control_east")
    selector.run()
